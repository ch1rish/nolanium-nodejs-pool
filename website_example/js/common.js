...
function formatPaymentLink(hash, merged){
    return `<a target="_blank" href="${getTransactionUrl(hash, merged)}"><span class="hashCode">${hash.substring(0, 15)}...${hash.substring(hash.length - 15, hash.length)}</span></a>`;
}
...
function formatLuck(difficulty, shares, solo=false) {
    ...
    if(!percent){
        ...
        return `<span class="luckGood">${percent.toLocaleString('en-US')}%&nbsp;</span>` + (solo === true ? `<span class="fa fa-user luckGood" title="Solo Mined"></span>` : ``);
    }
    else if(percent >= 101 && percent <= 150){
        return `<span class="luckMid">${percent.toLocaleString('en-US')}%&nbsp;</span>` + (solo === true ? `<span class="fa fa-user luckMid" title="Solo Mined"></span>` : ``);
    }
    else{
        return `<span class="luckBad">${percent.toLocaleString('en-US')}%&nbsp;</span>` + (solo === true ? `<span class="fa fa-user luckBad" title="Solo Mined"></span>` : ``);
    }
}
...
function poolBlocks_GenerateChart(data, displayedChart) {
    ...
    if (bgcolor === null) bgcolor = 'rgba(100, 151, 98, .4)';
    if (bordercolor === null) bordercolor = '#649762';
...
function getBlockRowElement(block, jsonString, stats){
    function formatBlockLink(hash, stats){
        return `<a target="_blank" href="${getBlockchainUrl(hash, stats)}"><span class="hashCode">${hash.substring(0, 10)}...${hash.substring(hash.length - 10, hash.length)}</span></a>`;
    }
    ...
    if(typeof block.reward == "undefined"){
        ...
        reward = getReadableCoin(stats, block.reward, null, true) + ' ' + stats.config.symbol;
    }
    //${block.hash.substring(0, 10)}...${block.hash.substring(block.hash.length - 10, block.hash.length)}
...
function top10Miners_GetMinerCells(position, data){
    ...
    return '<td class="col1" data-sort="' + position + '">' + position + '</td>' +
           ...
           '<td class="col5" data-sort="' + hashes + '">' + parseInt(hashes).toLocaleString('en-US') + '</td>';
}
...
function workerstats_FetchAddressStats(longpoll, stats, api, xhrAddressPoll){
    ...
    xhrAddressPoll[stats.config.coin] = $.ajax({
        ...
        success: function(data){
            ...
            updateText(`yourHashes${stats.config.coin}`, parseInt((data.stats.hashes || 0)).toLocaleString('en-US'));
            ...
            let shareProportion = Math.round(share_pct * 1000) / 1000;
            let scoreProportion = Math.round(score_pct * 1000) / 1000;
            updateText(`yourRoundShareProportion${stats.config.coin}`, (isNaN(shareProportion) ? '0' : shareProportion));
            updateText(`yourRoundScoreProportion${stats.config.coin}`, (isNaN(scoreProportion) ? '0' : scoreProportion));
            if (!lastStats.config.slushMiningEnabled) {
...
let home_GraphSettings = {
    ...
    height: '140',
    lineColor: '#649762',
    fillColor: 'rgba(100, 151, 98, .4)',
    spotColor: null,
    minSpotColor: null,
    maxSpotColor: null,
    highlightLineColor: '#649762',
    spotRadius: 3,
    chartRangeMin: 0,
