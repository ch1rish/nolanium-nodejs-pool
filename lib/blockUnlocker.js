					...
					apiInterfaces.rpcDaemon(rpcMethod, postRequest, function (error, result) {
						...
						apiInterfaces.rpcDaemon('f_transaction_json', {
							...
							if(blockHeight <= 259200) {
								...
								} else if(vout.length == 2) {
									block.reward = vout[0].amount;
									block.reward += vout[1].amount;
								}
							} else {
								if(vout.length == 3) {
									block.reward = vout[0].amount;
						...
						if(blockHeader.depth >= config.blockUnlocker.depth) {
							newUnlockedBlocks.push(block);
						}

						// Last item
						if(i == (blocks.length-1)) {
							if (newUnlockedBlocks.length === 0) {
								log('info', logSystem, 'No pending blocks are unlocked yet (%d pending)', [blocks.length]);
								callback(true);
								return
							}

							callback(null, newUnlockedBlocks)
						}
					});
				} else {
					postRequest = {
